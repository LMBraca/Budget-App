...
var CompoundIdField = class {
  constructor(...columns) {
    ...
    __publicField(this, "allowNull", false);
    __publicField(this, "dbReadOnly", false);
    __publicField(this, "isServerExpression", false);
...
function getEntityRef(entity, throwException = true) {
  ...
  if (!x2 && throwException)
    throw new Error("item " + (((_a = entity.constructor) == null ? void 0 : _a.name) || entity) + " was not initialized using a context");
  return x2;
}
...
function getRelationInfo(options) {
  return options == null ? void 0 : options[relationInfoMember];
}
...
var Filter = class _Filter {
  ...
  static async getPreciseValues(metadata, filter) {
    ...
    await _Filter.fromEntityFilter(metadata, filter).__applyToConsumer(result);
      ...
      if (Object.prototype.hasOwnProperty.call(whereItem, key)) {
        let fieldToFilter = whereItem[key];
        {
          if (key == "$or") {
            ...
            result.push(new NotFilter(_Filter.fromEntityFilter(entity, fieldToFilter)));
          } else if (key == "$and") {
            ...
            let fh = (rel == null ? void 0 : rel.type) === "toOne" ? op.fields ? new manyToOneFilterHelper(field, entity.fields, op) : new toOneFilterHelper(entity.fields[op.field]) : new filterHelper(field);
            let found = false;
  ...
  static async translateCustomWhere(r, entity, remult2) {
    let f2 = new customTranslator(async (filterKey, custom) => {
      let r2 = [];
      for (const key in entity.entityType) {
        ...
        if (element && element.rawFilterInfo && element.rawFilterInfo.rawFilterTranslator) {
          if (element.rawFilterInfo.key == filterKey) {
            r2.push(await _Filter.fromEntityFilter(entity, await element.rawFilterInfo.rawFilterTranslator(custom, remult2)));
          }
        }
      }
      return r2;
...
var filterHelper = class {
    ...
    if (ei) {
      if (val === void 0 || val === null) {
        if (val === null && !this.metadata.allowNull) {
          ...
          if ((rel == null ? void 0 : rel.type) === "reference")
  ...
  isIn(val) {
    ...
    if ((val == null ? void 0 : val.length) == 1 && val[0] != void 0 && val[0] !== null)
...
var manyToOneFilterHelper = class {
  constructor(metadata, fields, relationOptions) {
    __publicField(this, "metadata");
    __publicField(this, "fields");
    __publicField(this, "relationOptions");
    this.metadata = metadata;
    this.fields = fields;
...
var FilterSerializer = class _FilterSerializer {
  constructor() {
    ...
    __publicField(this, "hasUndefined", false);
  }
  ...
  not(filter) {
    let f2 = new _FilterSerializer();
    filter.__applyToConsumer(f2);
    this.add("NOT", f2.result);
  }
  isNull(col) {
...
function buildFilterFromRequestParameters(entity, filterInfo) {
  ...
  [...entity.fields].forEach((col) => {
    ...
    addFilter(".startsWith", (val2) => ({ $startsWith: val2 }), false, true);
    addFilter(".endsWith", (val2) => ({ $endsWith: val2 }), false, true);
  });
...
var customTranslator = class _customTranslator {
  constructor(translateCustom) {
    __publicField(this, "translateCustom");
  ...
  not(filter) {
    ...
    this.promises.push((async () => {
      ...
      await c.resolve();
      newNotElement = new Filter((x2) => c.applyTo(x2));
    })());
    this.commands.push((x2) => x2.not(newNotElement));
  }
  isEqualTo(col, val) {
  ...
  startsWithCaseInsensitive(col, val) {
    this.commands.push((x2) => x2.startsWithCaseInsensitive(col, val));
  }
  endsWithCaseInsensitive(col, val) {
    this.commands.push((x2) => x2.endsWithCaseInsensitive(col, val));
  }
  isIn(col, val) {
...
var preciseValuesCollector = class _preciseValuesCollector {
  constructor() {
    __publicField(this, "rawValues", {});
    __publicField(this, "preciseValues", new Proxy(this.rawValues, {
      get: (target, prop) => {
        if (prop in target) {
...
var RemultProxy = class {
  constructor() {
    /* @internal*/
    __publicField(this, "iAmRemultProxy", true);
    __publicField(this, "repoCache", /* @__PURE__ */ new Map());
    //@ts-ignore
    __publicField(this, "repo", (...args) => {
...
var LookupColumn = class {
  constructor(repository, isReferenceRelation, allowNull) {
    ...
    __publicField(this, "isReferenceRelation");
    __publicField(this, "allowNull");
    __publicField(this, "storedItem");
...
async function retry(what) {
  ...
  while (true) {
    try {
      ...
      if ((((_a = err.message) == null ? void 0 : _a.startsWith("Error occurred while trying to proxy")) || ((_b = err.message) == null ? void 0 : _b.startsWith("Error occured while trying to proxy")) || ((_c = err.message) == null ? void 0 : _c.includes("http proxy error")) || ((_d = err.message) == null ? void 0 : _d.startsWith("Gateway Timeout")) || err.status == 500) && i++ < flags.error500RetryCount) {
...
async function processHttpException(ex) {
  ...
  if (z.error)
    ...
    if (typeof ((_a = z.response) == null ? void 0 : _a.data) === "string")
      error = z.response.data;
    else
      error = (_b = z == null ? void 0 : z.response) == null ? void 0 : _b.data;
  }
  if (!error)
    error = z.message;
  ...
  if (httpStatusCode === void 0)
    httpStatusCode = (_c = z.response) == null ? void 0 : _c.status;
...
var RestDataProvider = class {
  constructor(apiProvider) {
    ...
    __publicField(this, "isProxy", true);
    this.apiProvider = apiProvider;
  }
  getEntityDataProvider(entity) {
    return new RestEntityDataProvider(() => {
      var _a;
      let url = (_a = this.apiProvider()) == null ? void 0 : _a.url;
    ...
    if (orderBy)
      for (const key in orderBy) {
        if (Object.prototype.hasOwnProperty.call(orderBy, key)) {
          ...
          if (field) {
            const rel = getRelationFieldInfo(field);
            if ((rel == null ? void 0 : rel.type) === "toOne") {
...
var LiveQuerySubscriber = class {
  constructor(repo2, query, userId) {
    ...
    __publicField(this, "queryChannel");
    __publicField(this, "subscribeCode");
    __publicField(this, "unsubscribe", () => {
    });
    __publicField(this, "defaultQueryState", []);
    __publicField(this, "listeners", []);
    __publicField(this, "id", v4_default());
    this.repo = repo2;
    this.query = query;
  ...
  async handle(messages) {
    {
      ...
      let loadedItems = await getRepositoryInternals(this.repo)._fromJsonArray(x2.map((m) => m.data.item), this.query.options);
...
function initDataProvider(optionsDataProvider, useStaticDefault, defaultDataProvider) {
  ...
  dataProvider = dataProvider.then(async (dp) => {
    ...
    if (dp)
      return dp;
    return defaultDataProvider == null ? void 0 : defaultDataProvider();
  });
  return dataProvider;
}
...
var RemultAsyncLocalStorage = class {
  constructor(remultObjectStorage) {
    __publicField(this, "remultObjectStorage");
  ...
  isInInitRequest() {
    var _a, _b;
    return (_b = (_a = this.remultObjectStorage) == null ? void 0 : _a.getStore()) == null ? void 0 : _b.inInitRequest;
  }
  setInInitRequest(val) {
    var _a;
    const store = (_a = this.remultObjectStorage) == null ? void 0 : _a.getStore();
    if (!store)
      return;
...
var Remult = class {
  constructor(provider) {
     ...
     */
    __publicField(this, "repo", (entity, dataProvider) => {
    ...
    __publicField(this, "dataProvider", new RestDataProvider(() => this.apiClient));
    /* @internal */
    __publicField(this, "repCache", /* @__PURE__ */ new Map());
    __publicField(this, "liveQueryStorage");
    __publicField(this, "subscriptionServer");
    /* @internal*/
    __publicField(this, "liveQueryPublisher", {
      itemChanged: async () => {
      }
    });
    /* @internal*/
    __publicField(this, "liveQuerySubscriber", new LiveQueryClient(() => this.apiClient, () => {
      var _a;
      return (_a = this.user) == null ? void 0 : _a.id;
    }));
  ...
  authenticated() {
    var _a;
    return ((_a = this.user) == null ? void 0 : _a.id) !== void 0;
  }
  ...
  isAllowed(roles) {
    ...
    if (typeof roles === "string") {
      if ((_b = (_a = this.user) == null ? void 0 : _a.roles) == null ? void 0 : _b.includes(roles.toString()))
        return true;
    }
    return false;
  }
...
__publicField(Remult, "onFind", (metadata, options) => {
});
/** A helper callback that is called whenever an entity is created. */
__publicField(Remult, "entityRefInit");
...
__publicField(Allow, "everyone", () => true);
__publicField(Allow, "authenticated", (...args) => {
  if (args.length > 1) {
...
var transactionLiveQueryPublisher = class {
  constructor(orig) {
    ...
    __publicField(this, "transactionItems", /* @__PURE__ */ new Map());
    this.orig = orig;
  }
...
async function withRemult(callback, options) {
  ...
  remult2.dataProvider = await initDataProvider(options == null ? void 0 : options.dataProvider, true, async () => remult2.dataProvider);
...
__publicField(InputTypes, "number", "number");
__publicField(InputTypes, "date", "date");
__publicField(InputTypes, "checkbox", "checkbox");
__publicField(InputTypes, "password", "password");
__publicField(InputTypes, "email", "email");
__publicField(InputTypes, "tel", "tel");
__publicField(InputTypes, "time", "time");

// node_modules/remult/esm/src/valueConverters.js
var _ValueConverters = class _ValueConverters {
};
__publicField(_ValueConverters, "Date", {
  toJson: (val) => {
    if (val === null)
      return null;
    if (!val)
...
__publicField(_ValueConverters, "DateOnly", {
...
__publicField(_ValueConverters, "DateOnlyString", {
  ..._ValueConverters.DateOnly,
  toDb: (d) => {
...
__publicField(_ValueConverters, "Boolean", {
  toDb: (val) => val,
...
__publicField(_ValueConverters, "Number", {
  fromDb: (value) => {
    if (value === null)
      return null;
    if (value !== void 0)
...
__publicField(_ValueConverters, "String", {
...
__publicField(_ValueConverters, "Integer", {
  ..._ValueConverters.Number,
  toJson: (value) => {
...
__publicField(_ValueConverters, "Default", {
  fromJson: (x2) => x2,
  toJson: (x2) => x2,
...
__publicField(_ValueConverters, "JsonString", {
  fromJson: (x2) => x2,
  toJson: (x2) => x2,
...
__publicField(_ValueConverters, "JsonValue", {
  fromJson: (x2) => x2,
  toJson: (x2) => x2,
  fromDb: (x2) => x2,
  toDb: (x2) => x2,
...
var ValueConverters = _ValueConverters;
...
var RelationLoader = class {
  constructor() {
    __publicField(this, "entityLoaders", /* @__PURE__ */ new Map());
    __publicField(this, "promises", []);
  }
...
var QueryVariation = class {
  constructor(rel) {
    __publicField(this, "rel");
    __publicField(this, "pendingInStatements", /* @__PURE__ */ new Map());
    __publicField(this, "whereVariations", /* @__PURE__ */ new Map());
    this.rel = rel;
  }
    ...
    try {
      ...
      for (const value of this.values.values()) {
        value.resolve(vals.filter((x2) => {
          ...
          const val = (rel == null ? void 0 : rel.type) === "reference" ? field.getId() : x2[this.key];
...
__publicField(_Validators, "required", createValidator(async (_, e) => e.value != null && e.value != void 0 && e.value !== "", "Should not be empty"));
__publicField(_Validators, "unique", createValidator(async (_, e) => {
  if (!e.entityRef)
 ...
 */
__publicField(_Validators, "uniqueOnBackend", createValidator(async (_, e) => {
  if (e.isBackend() && (e.isNew || e.valueChanged())) {
    return await e.entityRef.repository.count({
      [e.metadata.key]: e.value
    }) == 0;
...
__publicField(_Validators, "regex", createValueValidatorWithArgs((val, regex) => regex.test(val)));
__publicField(_Validators, "email", createValueValidator((val) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val), "Invalid Email"));
__publicField(_Validators, "url", createValueValidator((val) => !!new URL(val), "Invalid Url"));
__publicField(_Validators, "in", createValueValidatorWithArgs((val, values) => values.includes(val), (values) => `Value must be one of: ${values.map((y) => typeof y === "object" ? (y == null ? void 0 : y["id"]) !== void 0 ? y == null ? void 0 : y["id"] : y == null ? void 0 : y.toString() : y).join(", ")}`));
__publicField(_Validators, "notNull", createValueValidator((val) => val != null, "Should not be null"));
__publicField(_Validators, "enum", createValueValidatorWithArgs((value, enumObj) => Object.values(enumObj).includes(value), (enumObj) => `Value must be one of ${getEnumValues(enumObj).join(", ")}`));
__publicField(_Validators, "relationExists", createValidator(async (_, e) => {
  if (e.valueIsNull())
    return true;
  if (!e.isBackend())
    return true;
...
__publicField(_Validators, "maxLength", createValueValidatorWithArgs((val, maxLength) => val.length <= maxLength, (maxLength) => `Value must be at most ${maxLength} characters`));
__publicField(_Validators, "minLength", createValueValidatorWithArgs((val, minLength) => val.length >= minLength, (maxLength) => `Value must be at least ${maxLength} characters`));
__publicField(_Validators, "defaultMessage", "Invalid value");
var Validators = _Validators;
function createValidator(validate2, defaultMessage) {
  const validation = async (entity, e, message) => {
    const valid = await validate2(entity, e);
  ...
  const result = (entityOrMessage, e, message) => {
    if (typeof entityOrMessage === "string" || entityOrMessage === "function" || entityOrMessage === void 0 && e === void 0) {
      return async (entity, e2, message2) => await validation(entity, e2, entityOrMessage || message2);
    }
...
function createValidatorWithArgsInternal(validate2, defaultMessage, isValueValidator = false) {
  const result = (args, message) => async (entity, e) => {
    const valid = await validate2(entity, e, args);
...
var RepositoryImplementation = class _RepositoryImplementation {
  constructor(_entity, _remult, _dataProvider, _info, _defaultFindOptions) {
    ...
    __publicField(this, "_remult");
    __publicField(this, "_dataProvider");
    __publicField(this, "_info");
    __publicField(this, "_defaultFindOptions");
    __publicField(this, "__edp");
    __publicField(this, "_idCache", /* @__PURE__ */ new Map());
    __publicField(this, "listeners");
  ...
  async delete(item) {
    ...
    if (typeof item === "string" || typeof item === "number")
      if (this._dataProvider.isProxy)
        ...
        let ref22 = await this.findId(item);
        if (!ref22)
          throw this._notFoundError(item);
        return await getEntityRef(ref22).delete();
      }
  ...
  async validate(entity, ...fields) {
    {
      ...
      if (!fields || fields.length === 0) {
        ...
        for (const f2 of fields) {
          if (!await ref.fields.find(f2).validate())
            hasError = true;
        }
        if (!hasError)
          return void 0;
    ...
    return {
      subscribe: (l) => {
        ...
        return this._remult.liveQuerySubscriber.subscribe(
          //TODO - figure out why the type was required
          this,
          options,
          listener
        );
  ...
  async _loadManyToOneForManyRows(rawRows, loadOptions, loader) {
    ...
    if (loadOptions == null ? void 0 : loadOptions.load)
      ...
      if (ei) {
        ...
        if (!isRelation) {
          ...
          if (load) {
            ...
            if (toLoad.length > 0) {
              await loadManyToOne(repo2, toLoad);
            }
          }
        }
      }
    }
    async function loadManyToOne(repo2, toLoad) {
      let rows = await repo2.find({ where: repo2.metadata.idMetadata.getIdFilter(...toLoad) }, true);
      for (const r of rows) {
        repo2._addToCache(r);
      }
    }
    ...
    for (const col of this.metadata.fields) {
      ...
      const include = (_a = loadOptions == null ? void 0 : loadOptions.include) == null ? void 0 : _a[col.key];
      if (include !== void 0) {
        incl = include;
      }
      if (rel && incl) {
          ...
          if (returnNull)
            ...
            loader.load({
              ...
              if (result2.length == 0 && rel.type == "toOne")
                return;
              row[colKey] = rel.type !== "toMany" ? result2.length == 0 ? null : result2[0] : result2;
            });
          }
        }
      }
  ...
  async findOne(pOptions, skipOrderByAndLimit = false) {
    ...
    r = this.find({ ...options, limit: 1 }, skipOrderByAndLimit).then(async (items) => {
      ...
      if (!r2 && options.createIfNotFound) {
        r2 = this.create();
        if (options.where) {
          await __updateEntityBasedOnWhere(this.metadata, options.where, r2);
        }
      }
      return r2;
    });
  ...
  create(item) {
    ...
    if ((_a = this._defaultFindOptions) == null ? void 0 : _a.where) {
  ...
  async _translateWhereToFilter(where) {
    ...
    if ((_a = this._defaultFindOptions) == null ? void 0 : _a.where) {
      let z = safeWhere;
      safeWhere = {
        $and: [z, (_b = this._defaultFindOptions) == null ? void 0 : _b.where]
      };
    }
    if (!this._dataProvider.isProxy) {
...
var rowHelperBase = class {
  constructor(fieldsMetadata, instance, remult2, isNewRow) {
    __publicField(this, "fieldsMetadata");
    __publicField(this, "instance");
    __publicField(this, "remult");
    __publicField(this, "isNewRow");
    __publicField(this, "_error");
    __publicField(this, "_subscribers");
    __publicField(this, "_isLoading", false);
    __publicField(this, "lookups", /* @__PURE__ */ new Map());
      ...
      if (ei && remult2) {
        ...
        Object.defineProperty(instance, col.key, {
          ...
          set: (val2) => {
            var _a;
            lookup.set(val2);
            (_a = this._subscribers) == null ? void 0 : _a.reportChanged();
            if (!refImpl) {
              refImpl = this.fields.find(col.key);
              if (!refImpl._subscribers) {
                refImpl._subscribers = new SubscribableImp();
        ...
        if ((rel == null ? void 0 : rel.type) === "toOne") {
          ...
          Object.defineProperty(instance, col.key, {
            ...
            set: (newVal) => {
              ...
              if (op.field) {
                this.instance[op.field] = rel.toRepo.metadata.idMetadata.getId(newVal);
              }
              if (op.fields) {
  ...
  get error() {
    var _a;
    (_a = this._subscribers) == null ? void 0 : _a.reportObserved();
    return this._error;
  }
  set error(val) {
    var _a;
  ...
  async waitLoad() {
    await promiseAll([...this.lookups.values()], (x2) => x2.waitLoad());
  }
  __assertValidity() {
    if (!this.hasErrors())
  ...
  buildErrorInfoObject() {
    ...
    if (!error.message) {
      for (const col of this.fieldsMetadata) {
        if ((_a = this.errors) == null ? void 0 : _a[col.key]) {
  ...
  async _updateEntityBasedOnApi(body, ignoreApiAllowed = false) {
    ...
    await promiseAll([...this.fields].filter((f2) => !getRelationFieldInfo(f2.metadata)), (x2) => x2.load());
  }
};
var rowHelperImplementation = class extends rowHelperBase {
  constructor(info, instance, repo2, edp, remult2, _isNew) {
    super(info.fieldsMetadata, instance, remult2, _isNew);
    __publicField(this, "info");
    __publicField(this, "repo");
    __publicField(this, "edp");
    __publicField(this, "_isNew");
    __publicField(this, "repository");
    __publicField(this, "metadata");
    __publicField(this, "_wasDeleted", false);
    __publicField(this, "_columns");
    __publicField(this, "_saving", false);
    __publicField(this, "id");
    __publicField(this, "originalId");
    this.info = info;
    this.repo = repo2;
    this.edp = edp;
    this._isNew = _isNew;
    this.repository = repo2;
    this.metadata = info;
    if (_isNew) {
      for (const col of info.fieldsMetadata) {
        const colKey = col.key;
        if (col.options.defaultValue && instance[colKey] === void 0) {
  ...
  async reload() {
    await this.edp.find({ where: await this.getIdFilter() }).then(async (newData) => {
      if (newData.length === 0)
        ...
        if (!this.repo._dataProvider.isProxy) {
          ...
          if (this.repo.listeners)
            for (const listener of this.repo.listeners) {
              await ((_b = listener.saved) == null ? void 0 : _b.call(listener, this.instance, isNew));
            }
        }
        await this.repo._remult.liveQueryPublisher.itemChanged(this.repo.metadata.key, [{ id: this.getId(), oldId: this.getOriginalId(), deleted: false }]);
        this.saveOriginalData();
        this._isNew = false;
        return this.instance;
      } catch (err) {
  ...
  buildLifeCycleEvent(preventDefault = () => {
    ...
    return {
      ...
      metadata: self2.repo.metadata,
      repository: self2.repo,
      preventDefault: () => preventDefault(),
      relations: self2.repo.relations(self2.instance)
    };
  }
  async getIdFilter() {
  ...
  async delete() {
    ...
    try {
      ...
      if (this.repo.listeners)
        for (const listener of this.repo.listeners) {
          await ((_a = listener.deleted) == null ? void 0 : _a.call(listener, this.instance));
        }
      await this.repo._remult.liveQueryPublisher.itemChanged(this.repo.metadata.key, [{ id: this.getId(), oldId: this.getOriginalId(), deleted: true }]);
  ...
  async __performColumnAndEntityValidations() {
    ...
    if (this.repo.listeners)
      for (const listener of this.repo.listeners) {
        await ((_a = listener.validating) == null ? void 0 : _a.call(listener, this.instance));
      }
  }
};
...
function getControllerRef(container, remultArg) {
  ...
  if (!result) {
    ...
    container[controllerColumns] = result = new controllerRefImpl(prepareColumnInfo(columnSettings, remultVar).map((x2) => new columnDefsImpl(
      x2,
      void 0,
      //TODO - not sure
      remultVar
...
var FieldRefImplementation = class {
  constructor(settings, metadata, container, helper, rowBase) {
    ...
    __publicField(this, "rowBase");
    __publicField(this, "_subscribers");
    __publicField(this, "target");
    __publicField(this, "entityRef");
  ...
  get repo() {
    var _a;
    return (_a = this.helper) == null ? void 0 : _a.repository;
  }
  async load() {
    let lu = this.rowBase.lookups.get(this.metadata.key);
    ...
    if (rel && this.helper) {
      if (rel.type === "toMany") {
        ...
        let val = await this.repo.relations(this.container)[this.metadata.key].findOne();
        if (val)
  ...
  reportObserved() {
    var _a, _b;
    (_a = this._subscribers) == null ? void 0 : _a.reportObserved();
    (_b = this.rowBase._subscribers) == null ? void 0 : _b.reportObserved();
  }
  reportChanged() {
    var _a, _b;
    (_a = this._subscribers) == null ? void 0 : _a.reportChanged();
    (_b = this.rowBase._subscribers) == null ? void 0 : _b.reportChanged();
  }
  get error() {
    this.reportObserved();
    ...
    try {
      ...
      if (this.settings.validate) {
        ...
        let event = {
          ...
          isNew: ((_a = this.entityRef) == null ? void 0 : _a.isNew()) ?? false,
          load: () => self2.load(),
          ...
          isBackend: () => {
            var _a2, _b, _c;
            return !((_c = (_b = (_a2 = self2.rowBase) == null ? void 0 : _a2.remult) == null ? void 0 : _b.dataProvider) == null ? void 0 : _c.isProxy);
          }
        };
      ...
      if (typeof error === "string")
        ...
        this.error = error == null ? void 0 : error.message;
    }
  }
  async validate() {
...
var columnDefsImpl = class {
  constructor(settings, entityDefs, remult2) {
    __publicField(this, "settings");
    __publicField(this, "entityDefs");
    __publicField(this, "remult");
    __publicField(this, "options");
    __publicField(this, "target");
    __publicField(this, "readonly", false);
    __publicField(this, "valueConverter");
    __publicField(this, "allowNull");
    __publicField(this, "caption");
    __publicField(this, "dbName");
    __publicField(this, "inputType");
        ...
        if (typeof result === "function") {
          return (...args) => {
            try {
              ...
              const error = `${String(prop)} failed for value ${args == null ? void 0 : args[0]}. Error: ${typeof err === "string" ? err : err.message}`;
              throw {
...
var EntityFullInfo = class {
  constructor(columnsInfo, entityInfo2, remult2, entityType, key) {
    __publicField(this, "entityInfo");
    __publicField(this, "remult");
    __publicField(this, "entityType");
    __publicField(this, "key");
    __publicField(this, "options");
    __publicField(this, "fieldsMetadata", []);
    __publicField(this, "idMetadata", {
      getId: (item) => {
        if (item === void 0 || item === null)
          return item;
        const ref = getEntityRef(item, false);
...
function isAutoIncrement(f2) {
  var _a, _b;
  return ((_b = (_a = f2.options) == null ? void 0 : _a.valueConverter) == null ? void 0 : _b.fieldTypeInDb) === "autoincrement";
}
function ValueListFieldType(...options) {
  return (type, context) => {
    FieldType((o) => {
      ;
      o.valueConverter = ValueListInfo.get(type), o.displayValue = (item, val) => val == null ? void 0 : val.caption;
...
var ValueListInfo = class _ValueListInfo {
  constructor(valueListType) {
    __publicField(this, "valueListType");
    __publicField(this, "byIdMap", /* @__PURE__ */ new Map());
    __publicField(this, "values", []);
    __publicField(this, "isNumeric", false);
    __publicField(this, "fieldTypeInDb");
    __publicField(this, "inputType");
    ...
    if (options) {
      for (const op of options) {
        if (op == null ? void 0 : op.getValues) {
...
function getValueList(type) {
  let meta = type == null ? void 0 : type.metadata;
  if (!meta && isOfType(type, "options"))
    meta = type;
  type = (meta == null ? void 0 : meta.valueType) || type;
  if (type) {
    var options = type[storableMember];
    if (options)
      return ValueListInfo.get(type).getValues();
  }
  let optionalValues = (meta == null ? void 0 : meta.options)[fieldOptionalValuesFunctionKey];
  if (optionalValues)
  ...
  for (const o of options) {
    if (o) {
      if (typeof o === "function")
        ...
        const { validate: validate2, ...otherOptions } = o;
...
function decorateColumnSettings(settings, remult2) {
  ...
  if (!settings.valueConverter.toDb) {
    settings.valueConverter.toDb = (x2) => toJson(x2);
  }
  if (!settings.valueConverter.fromDb) {
    settings.valueConverter.fromDb = (x2) => fromJson(x2);
  }
  if (!settings.valueConverter.toInput) {
    settings.valueConverter.toInput = (x2) => toJson(x2);
  }
  if (!settings.valueConverter.fromInput) {
    settings.valueConverter.fromInput = (x2) => fromJson(x2);
  }
  return settings;
}
    ...
    itStrategy = async () => {
      ...
      if (currentPage === void 0 || itemIndex == currentPage.items.length) {
        ...
        if (currentPage.items.length == 0) {
          ...
          if ((prev == null ? void 0 : prev.items.length) ?? 0 > 0) {
...
var _SqlDatabase = class _SqlDatabase {
  /**
  ...
  constructor(sql) {
     ...
     */
    __publicField(this, "wrapIdentifier", (x2) => x2);
    __publicField(this, "provideMigrationBuilder");
    __publicField(this, "createdEntities", []);
   ...
   */
  transaction(action) {
    return this.sql.transaction(async (x2) => {
      let completed = false;
      try {
        await action(new _SqlDatabase({
          createCommand: () => {
   ...
   */
  static async filterToRaw(repo2, condition, sqlCommand, dbNames, wrapIdentifier) {
    if (!sqlCommand) {
      sqlCommand = new myDummySQLCommand();
    }
    const r = getRepository(repo2);
    var b = new FilterConsumerBridgeToSqlRequest(sqlCommand, dbNames || await dbNamesOfWithForceSqlExpression(r.metadata, wrapIdentifier));
    b._addWhere = false;
 ...
 */
__publicField(_SqlDatabase, "LogToConsole", false);
/**
 * Threshold in milliseconds for logging queries to the console.
 */
__publicField(_SqlDatabase, "durationThreshold", 0);
var SqlDatabase = _SqlDatabase;
var icons = /* @__PURE__ */ new Map([
  // CRUD
  ["INSERT", "⚪"],
  // Used to insert new data into a database.
  ["SELECT", "🔵"],
  // Used to select data from a database and retrieve it.
  ["UPDATE", "🟣"],
  // Used to update existing data within a database.
  ["DELETE", "🟤"],
  // Used to delete existing data from a database.
  // Additional
  ["CREATE", "🟩"],
  ...
  ["DROP", "🟥"],
  // Used to delete an entire table or database.
  ["TRUNCATE", "⬛"],
  // Used to remove all records from a table, including all spaces allocated for the records are removed.
  ["GRANT", "🟪"],
  // Used to give a specific user permission to perform certain tasks.
  ["REVOKE", "🟫"]
...
var LogSQLCommand = class {
  constructor(origin, logToConsole) {
    ...
    __publicField(this, "logToConsole");
...
var ActualSQLEntityDataProvider = class {
  constructor(entity, sql, iAmUsed, strategy) {
    __publicField(this, "entity");
    __publicField(this, "sql");
    __publicField(this, "iAmUsed");
  ...
  async init() {
    let dbNameProvider = await dbNamesOfWithForceSqlExpression(this.entity, (x2) => this.sql.wrapIdentifier(x2));
  ...
  async update(id, data) {
    ...
    statement += await f2.resolveWhere();
    let { colKeys, select } = this.buildSelect(e);
    let returning = true;
    if (this.sql._getSourceSql().doesNotSupportReturningSyntax)
      returning = false;
    if (returning && this.sql._getSourceSql().doesNotSupportReturningSyntaxOnlyForUpdate)
      returning = false;
    if (returning)
      statement += " returning " + select;
    return r.execute(statement).then((sqlResult) => {
      var _a, _b;
      (_b = (_a = this.sql._getSourceSql()).afterMutation) == null ? void 0 : _b.call(_a);
      if (!returning) {
...
__publicField(ActualSQLEntityDataProvider, "LogToConsole", false);
...
var FilterConsumerBridgeToSqlRequest = class _FilterConsumerBridgeToSqlRequest {
  constructor(r, nameProvider) {
    ...
    __publicField(this, "nameProvider");
    __publicField(this, "where", "");
    __publicField(this, "_addWhere", true);
    __publicField(this, "promises", []);
    this.r = r;
  ...
  or(orElements) {
    ...
    this.promises.push((async () => {
      for (const element of orElements) {
        ...
        let where = await f2.resolveWhere();
        if (!where)
          return;
        if (where.length > 0) {
  ...
  databaseCustom(databaseCustom) {
    this.promises.push((async () => {
      if (databaseCustom == null ? void 0 : databaseCustom.buildSql) {
...
var CustomSqlFilterBuilder = class {
  constructor(r, wrapIdentifier) {
    __publicField(this, "r");
    __publicField(this, "wrapIdentifier");
    __publicField(this, "sql", "");
    /**
     ...
     */
    __publicField(this, "param", (val, field) => {
     ...
     */
    __publicField(this, "filterToRaw", async (repo2, condition) => {
...
async function dbNamesOf(repo2, wrapIdentifierOrOptions) {
  return internalDbNamesOf(repo2, wrapIdentifierOrOptions);
}
async function dbNamesOfWithForceSqlExpression(repo2, wrapIdentifierOrOptions) {
  return internalDbNamesOf(repo2, wrapIdentifierOrOptions, true);
}
async function internalDbNamesOf(repo2, wrapIdentifierOrOptions, forceSqlExpression = false) {
...
async function entityDbName(metadata, wrapIdentifier = (x2) => x2) {
...
async function fieldDbName(f2, meta, wrapIdentifier = (x2) => x2, forceSqlExpression = false) {
  try {
    if (f2.options.sqlExpression) {
      let result;
      if (typeof f2.options.sqlExpression === "function") {
        if (f2[sqlExpressionInProgressKey] && !forceSqlExpression) {
          return "recursive sqlExpression call for field '" + f2.key + "'. ";
        }
        try {
          ;
          f2[sqlExpressionInProgressKey] = true;
          result = await f2.options.sqlExpression(meta);
          f2.options.sqlExpression = () => result;
        } finally {
          delete f2[sqlExpressionInProgressKey];
        }
      } else
    ...
    let field = (rel == null ? void 0 : rel.type) === "toOne" && f2.options.field;
    if (field) {
...
var ArrayEntityDataProvider = class {
  ...
  async init() {
    if (this.__names)
      return this.__names;
    this.__names = await dbNamesOf(this.entity, (x2) => x2);
  ...
  return (target, info) => {
    for (const rawFilterMember in target) {
      if (Object.prototype.hasOwnProperty.call(target, rawFilterMember)) {
        ...
        if (element == null ? void 0 : element.rawFilterInfo) {
...
var _Action = class _Action {
  constructor(actionUrl, queue, allowed) {
    __publicField(this, "actionUrl");
    __publicField(this, "queue");
    __publicField(this, "allowed");
    __publicField(this, "doWork");
      ...
      try {
        ...
        await remultStatic.actionInfo.runActionWithoutBlockingUI(async () => {
          while (!runningJob || !runningJob.done) {
            ...
            runningJob = await http.post(baseUrl + "/" + _Action.apiUrlForJobStatus, { queuedJobId: r.queuedJobId });
...
__publicField(_Action, "apiUrlForJobStatus", "jobStatusInQueue");
var Action = _Action;
var ForbiddenError = class extends Error {
  constructor(message = "Forbidden") {
    super(message);
    __publicField(this, "isForbiddenError", true);
  }
};
var myServerAction = class extends Action {
  ...
  async execute(info, remult2, res) {
    let result = { data: {} };
    let ds = remult2.dataProvider;
    await doTransaction(remult2, async () => {
      if (!remult2.isAllowedForInstance(void 0, this.options.allowed))
        throw new ForbiddenError();
      info.args = await prepareReceivedArgs(this.types(), info.args, remult2, ds, res);
      try {
...
function BackendMethod(options) {
  return (target, context, descriptor) => {
    ...
    if (target.prototype !== void 0) {
      let serverAction2 = new myServerAction(((options == null ? void 0 : options.apiPrefix) ? options.apiPrefix + "/" : "") + key, () => getTypes(), options, (args) => originalMethod.apply(void 0, args));
      serverAction2.doWork = async (args, self2, url, http) => {
        args = prepareArgsToSend(getTypes(), args);
        if (options.blockUser === false) {
          return await remultStatic.actionInfo.runActionWithoutBlockingUI(async () => (await serverAction2.run({ args }, url, http)).data);
        } else
          return (await serverAction2.run({ args }, url, http)).data;
      };
      result = async function(...args) {
        if (!isBackend()) {
          return await serverAction2.doWork(args, void 0);
        } else
    ...
    let serverAction = {
      __register(reg) {
        ...
        for (const constructor of x2.classes.keys()) {
          ...
          reg(controllerOptions.key + "/" + ((options == null ? void 0 : options.apiPrefix) ? options.apiPrefix + "/" : "") + key, options ? options.queue ?? false : false, options.allowed, async (d, req, res) => {
            ...
            try {
              ...
              let r;
              await doTransaction(remult2, async () => {
                d.args = await prepareReceivedArgs(getTypes(), d.args, remult2, remult2.dataProvider, res);
                if (remultStatic.allEntities.includes(constructor)) {
                  ...
                  if (rowInfo.isNewRow) {
                    ...
                    let rows = await repo2.find({
                      where: {
                        ...repo2.metadata.idMetadata.getIdFilter(rowInfo.id),
                    ...
                    y = rows[0];
                    await repo2.getEntityRef(y)._updateEntityBasedOnApi(rowInfo.data);
                  }
      ...
      doWork: async function(args, self2, baseUrl, http) {
        args = prepareArgsToSend(getTypes(), args);
        if (remultStatic.allEntities.includes(target.constructor)) {
          ...
          try {
            ...
            }(classOptions2.key + "/" + ((options == null ? void 0 : options.apiPrefix) ? options.apiPrefix + "/" : "") + key, (options == null ? void 0 : options.queue) ?? false, options.allowed).run({
              args,
              rowInfo: {
                data: await defs.toApiJson(),
                isNewRow: defs.isNew(),
          ...
          try {
            ...
            }(x2.classes.get(self2.constructor).key + "/" + ((options == null ? void 0 : options.apiPrefix) ? options.apiPrefix + "/" : "") + key, (options == null ? void 0 : options.queue) ?? false, options.allowed).run({
              args,
    ...
    result = async function(...args) {
      ...
      if (!isBackend()) {
        ...
        return await originalMethod.apply(self2, args);
    };
...
async function prepareReceivedArgs(types, args, remult2, ds, res) {
      ...
      if (types[i] == Remult || types[i] == Remult) {
        ...
        if (eo != null) {
          if (!(args[i] === null || args[i] === void 0))
            args[i] = await remult2.repo(types[i]).findId(args[i]);
        }
      }
    }
  return args;
...
var JsonEntityDataProvider = class {
  ...
  async loadEntityData(what) {
    ...
    return what(dp, async () => await this.helper.setItem(dbName, JSON.stringify(data, void 0, this.formatted ? 2 : void 0)));
  }
  find(options) {
  ...
  update(id, data) {
    return this.p = this.p.then(() => this.loadEntityData((dp, save) => dp.update(id, data).then(async (x2) => {
      await save();
      return x2;
    })));
  }
  delete(id) {
    return this.p = this.p.then(() => this.loadEntityData((dp, save) => dp.delete(id).then(async (x2) => {
      await save();
      return x2;
    })));
  }
  async insert(data) {
    return this.p = this.p.then(() => this.loadEntityData((dp, save) => dp.insert(data).then(async (x2) => {
      await save();
      return x2;
    })));
  }
...
var JsonEntityOpfsStorage = class {
  ...
  async getItem(entityDbName2) {
    const opfsFile = await (await this.init()).getFileHandle(entityDbName2 + ".json", {
      create: true
    });
  ...
  async setItem(entityDbName2, json) {
    const opfsFile = await await (await this.init()).getFileHandle(entityDbName2 + ".json", {
      create: true
    });
...
var InMemoryLiveQueryStorage = class {
  constructor() {
    __publicField(this, "debugFileSaver", (x2) => {
    });
    __publicField(this, "queries", []);
    __publicField(this, "removeCountForTesting", 0);
  }
  debug() {
  ...
  async add(query) {
    this.queries.push({ ...query, lastUsed: (/* @__PURE__ */ new Date()).toISOString() });
    this.debug();
  }
  async remove(id) {
